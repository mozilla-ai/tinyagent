"""Integration tests for `LlmJudge` and `AgentJudge`."""
...
from tinyagent import AgentTrace
from tinyagent.evaluation.agent_judge import AgentJudge
from tinyagent.evaluation.llm_judge import LlmJudge
from tinyagent.evaluation.schemas import EvaluationOutput
from tinyagent.testing.helpers import get_default_agent_model_args
...
def test_llm_judge(agent_trace: AgentTrace) -> None:
    ...
    assert result1.passed, (
        f"Expected the messages to contain the year 2025, but evaluation failed: {result1.reasoning}"
    )
...
def test_agent_judge(agent_trace: AgentTrace) -> None:
    ...
    eval_trace = agent_judge.run(
        trace=agent_trace,
        question=(
            "Did the agent write the year to a file? "
            "Grab the messages from the trace and check if the write_file tool was called."
        ),
